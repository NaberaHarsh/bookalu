import { singleImgType, Payload } from "./ImgUploadType";
...
export const imgUpload: AppThunk = (payload: Payload) => {
  return (dispatch: Dispatch) => {
    // On initial Call Start The loader
    dispatch({
      type: singleImgType.FETCH_REQUEST,
    ...
    axios
      ...
      .then((res) => {
        dispatch({
          type: singleImgType.FETCH_SUCCESS,
      ...
      .catch((err) => {
        dispatch({
          type: singleImgType.FETCH_ERROR,
          data: err.response,
        });
      });
  };
