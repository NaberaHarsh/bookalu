...
export const requiredCounty = (value:any) => {
  return (!value || value === "" || value.trim() === "") ? "Required": undefined
};
  
export const requiredCompanyInfo = (value: any) =>
...
export const characterCount250 = (value: any) =>
  value && value.length > 250 && "Only 250 Characters are allowed";

export const minimum5char = (value: any) =>
  value && value.length < 5 && "At least 5 characters are required";
...
export const isUserNameExists = simpleMemoize(async (value: any) => {
  if (value) {
    try {
      // const response =
       await userService.getRequest(
        `businessAccount/checkUserName/${value}`
      );
...
export const postCodeValidation = (values: any) => {
  if (!(/^([A-Za-z][A-Ha-hJ-Yj-y]?[0-9][A-Za-z0-9]? ?[0-9][A-Za-z]{2}|[Gg][Ii][Rr] ?0[Aa]{2})$/.test(values))) return "Invalid post code";
}

export const noMoreThan10 = (value: any) => {
  return value && value.length > 10 && "Maximum 10 occupations can be added";
};

export const timeRequired = (value: any) => {
  return (!value || (value && value === '00:00')) ? 'Required' : undefined;
}